# -*- coding: utf-8 -*-
"""
intfloat/multilingual-e5-large を Hugging Face から取得して使用可能にするラッパ。

参考:
- src/WeatherLLM/llava.py のローカル保存方針（snapshot_download によるローカル保存、存在時は再DLしない）
- 既存実装: model/Multilingual_E5_base.py（インターフェースを踏襲）
- モデルカード: https://huggingface.co/intfloat/multilingual-e5-large
  - 推奨フォーマット: クエリ側は "query: ...", ドキュメント側は "passage: ..."
  - Sentence-Transformers 形式 / 1024 次元（large） / 最大長 ~512

提供関数:
- get_model(device: str = "cpu", local_dir: Optional[str] = None)
    -> SentenceTransformer
- add_prefix_for_task(text: str, side: str, task: str = "sts") -> str
    E5 推奨のプレフィックス（sts は両方 query を使用）
...
LOGGER = setup_logger("multilingual-e5-large")
...
def _ensure_local_model_dir(
    ...
    if _has_model_files(local_dir):
        LOGGER.info(f"[e5-large] local snapshot found. skip download: {local_dir}")
        return local_dir

    if not HAS_HF_HUB:
        LOGGER.warning(
            "[e5-large] huggingface_hub が見つからないため、ローカル保存はスキップします。"
            "オンラインで SentenceTransformer から直接読み込みます。"
        )
        return model_id

    os.makedirs(local_dir, exist_ok=True)
    LOGGER.info(f"[e5-large] downloading snapshot to: {local_dir}")
    snapshot_download(
        repo_id=model_id,
        local_dir=local_dir,
        token=hf_token,
...
def get_model(
    ...
    """
    multilingual-e5-large モデルをロードして返す。
    ...
    LOGGER.info(f"[e5-large] model loaded from: {path_or_id} (device={model.device})")
    return model
...
if __name__ == "__main__":
    ...
    print(f"[self-test] multilingual-e5-large similarity(STS)={sim:.4f}")

# -*- coding: utf-8 -*-
"""
answerdotai/JaColBERTv2.5 をローカル環境で使用可能にするラッパ。
...
環境変数（任意）:
- JACOLBERT_V2_5_ID         : 既定モデルIDを上書き（デフォルト: 'answerdotai/JaColBERTv2.5'）
- JACOLBERT_V2_5_LOCAL_DIR  : 既にダウンロード済みのローカル ST 形式ディレクトリを明示
"""
...
LOGGER = setup_logger("JaColBERTv2.5")
...
DEFAULT_MODEL_ID = "answerdotai/JaColBERTv2.5"


def _resolve_model_id() -> str:
    mid = os.getenv("JACOLBERT_V2_5_ID", "").strip()
...
def _ensure_local_model_dir(
    ...
    """
    ローカル保存を確保（存在すればスキップ）。
    - 環境変数 JACOLBERT_V2_5_LOCAL_DIR が ST 形式として有効ならそれを優先
    - huggingface_hub が無ければ model_id を返し、ライブラリ側のオンラインロードに委譲
    """
    # 環境変数でローカル指定があれば優先
    env_local = os.getenv("JACOLBERT_V2_5_LOCAL_DIR", "").strip()
    if env_local and _has_st_dir(env_local):
        LOGGER.info(f"[JaColBERTv2.5] Use local dir from env: {env_local}")
        return env_local
    elif env_local:
        LOGGER.warning(f"[JaColBERTv2.5] JACOLBERT_V2_5_LOCAL_DIR は ST 形式として不正です: {env_local}")
    ...
    if _has_st_dir(local_dir):
        LOGGER.info(f"[JaColBERTv2.5] local snapshot found. skip download: {local_dir}")
        return local_dir

    if not HAS_HF_HUB:
        LOGGER.warning(
            "[JaColBERTv2.5] huggingface_hub が見つからないため、ローカル保存はスキップします。"
            "バックエンド側のオンライン読み込みに委譲します。"
        )
        return model_id

    os.makedirs(local_dir, exist_ok=True)
    LOGGER.info(f"[JaColBERTv2.5] downloading snapshot to: {local_dir} (repo_id={model_id})")
    try:
        snapshot_download(
            repo_id=model_id,
            local_dir=local_dir,
        ...
        LOGGER.warning(f"[JaColBERTv2.5] snapshot_download failed: {e}. Fallback to online load: {model_id}")
        return model_id
    return local_dir
...
def _load_model(
    ...
    if HAS_ST and isinstance(path_or_id, str) and os.path.isdir(path_or_id) and _has_st_dir(path_or_id):
        try:
            ...
            LOGGER.info(f"[JaColBERTv2.5] ST backend loaded from: {path_or_id} (device={st_model.device})")
            return _ModelBundle(backend="st", device=torch_device, st_model=st_model)
        except Exception as e:
            LOGGER.warning(f"[JaColBERTv2.5] ST backend load failed, fallback to HF: {e}")

    # 2) Transformers バックエンド
    if not HAS_HF:
        raise RuntimeError(
            "transformers が見つからないため JaColBERTv2.5 をロードできません。\n"
    ...
    try:
        ...
        raise RuntimeError(
            "JaColBERTv2.5 のモデルリポジトリにアクセスできませんでした。"
            " 環境変数 JACOLBERT_V2_5_ID（正しい HF モデルID）"
            " または JACOLBERT_V2_5_LOCAL_DIR（ローカルのモデルディレクトリ）を設定してください。"
        ) from e
    LOGGER.info(f"[JaColBERTv2.5] HF backend loaded from: {path_or_id} (device={torch_device})")
...
def _encode_st(
    ...
    if isinstance(text, str):
        text = [text]
    # ST 5.x: normalize_embeddings が利用可能
    try:
...
def _token_embeddings_hf(
    ...
    if tok_embs.numel() == 0:
        # すべて特殊トークンしか残らない場合は [CLS] のみでも返す
        tok_embs = hidden[attn]
        if tok_embs.numel() == 0:
            tok_embs = hidden  # 最終フォールバック
    return tok_embs
...
def embed_and_score(
    ...
    raise RuntimeError("JaColBERTv2.5 をロードできませんでした。transformers または sentence-transformers の環境を確認してください。")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[self-test] JaColBERTv2.5 similarity={s:.4f}")
    except Exception as e:

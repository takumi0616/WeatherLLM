# -*- coding: utf-8 -*-
"""
cl-nagoya/ruri-v3-310m を Hugging Face から取得して使用可能にするラッパ。

参考:
- src/WeatherLLM/llava.py のローカル保存方針
- 同梱 PDF（ruri-v3-310m モデルカードの抜粋）
  - SentenceTransformer 形式 / 出力次元 768 / 最大長 8192
  - 1+3 prefix scheme:
      ""（空文字）: 一般的な意味表現（STS 等）
      "トピック: "    : 分類/クラスタリング/トピック
      "検索クエリ: "  : 検索クエリ（retrieval/reranking の query 側）
      "検索文書: "    : 検索文書（retrieval/reranking の doc 側）

提供関数:
- get_model(device: str = "cpu", local_dir: Optional[str] = None)
    -> SentenceTransformer
- add_prefix_for_task(text: str, side: str, task: str = "sts") -> str
    Ruri v3 推奨のフォーマットに従い、タスク/サイドに応じてプレフィックスを付与
...
- embed_and_score(text_a: str, text_b: str, device: str = "cpu",
    ...
    2文の類似度（コサイン類似度）を返す。STS では両文とも空プレフィックス（素の意味表現）。

注意:
- オフラインでも使えるように、存在しなければ huggingface_hub.snapshot_download でローカルに保存
- HF_TOKEN 環境変数が設定されていればそれを使用
- 環境要件: Transformers >= 4.48.0（本環境 4.57.0 なので満たす）、Sentence-Transformers 5.1.1
"""
...
LOGGER = setup_logger("ruri-v3-310m")
...
def _ensure_local_model_dir(
    ...
    if _has_model_files(local_dir):
        LOGGER.info(f"[ruri-v3] local snapshot found. skip download: {local_dir}")
        return local_dir

    if not HAS_HF_HUB:
        LOGGER.warning(
            "[ruri-v3] huggingface_hub が見つからないため、ローカル保存はスキップします。"
            "オンラインで SentenceTransformer から直接読み込みます。"
        )
        return model_id

    os.makedirs(local_dir, exist_ok=True)
    LOGGER.info(f"[ruri-v3] downloading snapshot to: {local_dir}")
    snapshot_download(
        repo_id=model_id,
        local_dir=local_dir,
        token=hf_token,
...
def get_model(
    ...
    """
    cl-nagoya/ruri-v3-310m モデルをロードして返す。
    ...
    # 768 次元 / 最大長 8192（モデルカードより）
    LOGGER.info(f"[ruri-v3] model loaded from: {path_or_id} (device={model.device})")
    return model


def add_prefix_for_task(text: str, side: str, task: str = "sts") -> str:
    """
    タスク別に Ruri v3 推奨のプレフィックスを付与する。
    - side: "query" / "doc" を想定（retrieval/reranking で利用）
    ...
    if task in {"clustering", "classification"}:
        return f"{TOPIC_PREFIX}{text}"

    # その他タスクはそのまま（必要に応じて拡張）
    return text
...
def embed_and_score(
    ...
    - STS タスクでは、両文とも空プレフィックス（素の意味表現）
    - retrieval/reranking では query/doc を分ける
    - classification/clustering では「トピック: 」プレフィックス
    """
    ...
    elif task in {"retrieval", "reranking"}:
...
if __name__ == "__main__":
    # 簡易自己テスト（CPU）
    qa = "瑠璃色はどんな色？"
    qb = "瑠璃色（るりいろ）は、紫みを帯びた濃い青のこと。"

    sim = embed_and_score(qa, qb, device="cpu", task="sts")
    print(f"[self-test ruri-v3-310m] similarity(STS)={sim:.4f}")

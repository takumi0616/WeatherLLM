# -*- coding: utf-8 -*-
"""
cl-nagoya/ruri-base を Hugging Face から取得して使用可能にするラッパ。

参考:
- src/WeatherLLM/llava.py のローカル保存方針
- モデルカード要点（ruri-base）: SentenceTransformer 形式 / 出力次元 768 / 最大長 512
  - クエリ側には「クエリ: 」プレフィックス
  - パッセージ（ドキュメント）側には「文章: 」プレフィックス

提供関数:
- get_model(device: str = "cpu", local_dir: Optional[str] = None)
    -> SentenceTransformer
- add_prefix_for_task(text: str, side: str, task: str = "sts") -> str
    Ruri 推奨のフォーマットに従い、クエリ/ドキュメント側へプレフィックスを付与
...
- embed_and_score(text_a: str, text_b: str, device: str = "cpu",
    ...
    2文の類似度（コサイン類似度）を返す。STS タスクでは両文をクエリ形式にするのが簡便。
...
# main_v1.py と整合のために同じタスク名を許容（厳密なテンプレートはモデルカード未定義のものもある）
...
def _ensure_local_model_dir(
    ...
    if _has_model_files(local_dir):
        LOGGER.info(f"[ruri] local snapshot found. skip download: {local_dir}")
        return local_dir

    if not HAS_HF_HUB:
        LOGGER.warning(
            "[ruri] huggingface_hub が見つからないため、ローカル保存はスキップします。"
            "オンラインで SentenceTransformer から直接読み込みます。"
        )
        return model_id

    os.makedirs(local_dir, exist_ok=True)
    LOGGER.info(f"[ruri] downloading snapshot to: {local_dir}")
    snapshot_download(
        repo_id=model_id,
        local_dir=local_dir,
        token=hf_token,
...
def get_model(
    ...
    """
    cl-nagoya/ruri-base モデルをロードして返す。
    ...
    LOGGER.info(f"[ruri] model loaded from: {path_or_id} (device={model.device})")
    return model


def add_prefix_for_task(text: str, side: str, task: str = "sts") -> str:
    """
    タスク別に Ruri 推奨のプレフィックスを付与する。
    ...
    # その他のタスクではテンプレ未定義のため、そのまま返す（必要に応じて拡張可）
    return text

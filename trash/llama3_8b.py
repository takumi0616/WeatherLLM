# -*- coding: utf-8 -*-
"""
Llama 3 系（既定 8B Instruct）をローカルPython環境で実行するためのスクリプト。
「単一コマンドでの一連処理（ダウンロード→軽い推論→LoRA微調整→推論）」を追加。

- 既に保存済みであればスキップ（モデルの再ダウンロード/再学習は行わない）
- すべてローカル保存物でオフライン推論可能
- LoRA はデフォルトで QLoRA（4bit）を用いて省メモリ学習
- GPU メモリ不足対策として推論時も4bit量子化を使用可能

実行コマンド（カレントが src/WeatherLLM の想定）
```shell
# 1コマンドで一連の処理（GPU, QLoRA, 自動スキップ対応）
notify-run wsl-ubuntu -- nohup python llama3_8b.py --run-all --device cuda --max-new-tokens 2048 --epochs 10 --batch-size 2 --tag lora_all > llama3_8b.log 2>&1 &

# メモリ制限がある場合は --use-4bit-inference オプションを追加
notify-run wsl-ubuntu -- nohup python llama3_8b.py --run-all --device cuda --use-4bit-inference --max-new-tokens 2048 --epochs 10 --batch-size 2 --tag lora_all > llama3_8b.log 2>&1 &
```
"""
...
def preprocess_for_causal_lm(examples, tokenizer, max_length: int = 1024):
    ...
    for instruction, response in zip(examples["instruction"], examples["response"]):
        # chat_template を使わず、シンプルなテキスト結合で学習デモ（LoRA対象）
...
def ensure_local_model_dir(model_id: str, local_dir: str, hf_token: Optional[str]) -> None:
    ...
    if HAS_HF_HUB:
        ...
        tmp_model = AutoModelForCausalLM.from_pretrained(
            model_id, device_map="cpu", dtype=torch.float32, trust_remote_code=True, low_cpu_mem_usage=True, token=hf_token
        )
...
def load_model_and_tokenizer_from_local(
    ...
    Args:
        use_4bit_inference: 推論時にも4bit量子化を使用するかどうか（メモリ不足対策）
    """
    quant_cfg = None
    torch_dtype_arg = dtype
    
    # 学習時または4bit推論時に量子化を使用
    if (for_training and use_qlora) or use_4bit_inference:
        # QLoRA: 4bit でロード
        ...
        torch_dtype_arg = None  # quantization_config と併用時は dtype=None にする
    ...
    try:
        ...
        pass
    # Llama3 の <|eot_id|> など終端設定
    try:
        eot_id = tokenizer.convert_tokens_to_ids("<|eot_id|>")
        if isinstance(eot_id, int) and eot_id != tokenizer.unk_token_id:
            model.generation_config.eos_token_id = [tokenizer.eos_token_id, eot_id]
    except Exception:
        pass
...
def train_lora_and_save(
    ...
    # torchrun を使わず python 実行時に複数GPUがある場合は DataParallel で並列化
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Llama 3 locally (with optional QLoRA).")
...
def run_all_pipeline(args: argparse.Namespace) -> None:
    ...
    if torch.cuda.is_available():
        ...
        if total_memory < 20:  # 20GB未満のGPUの場合は警告
            LOGGER.warning(f"Limited GPU memory detected ({total_memory:.2f} GiB). Using 4-bit quantization for inference.")
            args.use_4bit_inference = True
    ...
    # 保存確保（DDP: rank0のみダウンロードし、他rankは同期してから参照）
    ...
    messages2 = [{"role": "system", "content": "日本語で丁寧に回答してください。"}, {"role": "user", "content": "富士山の高さは？"}]
...
def main():
    ...
    # 従来の単発生成（互換維持）
    torch.manual_seed(args.seed)
    out_dir = make_output_dir("outputs", args.model, args.tag)
    os.makedirs(out_dir, exist_ok=True)
    save_json(os.path.join(out_dir, "run_args.json"), vars(args))
    ...
    if torch.cuda.is_available():
        ...
        total_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
        LOGGER.info(f"GPU Total Memory: {total_memory:.2f} GiB")
        if total_memory < 20 and not args.use_4bit_inference:
            LOGGER.warning(f"Limited GPU memory detected ({total_memory:.2f} GiB). Consider using --use-4bit-inference flag.")

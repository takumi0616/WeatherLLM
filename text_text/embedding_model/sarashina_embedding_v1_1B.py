# -*- coding: utf-8 -*-
"""
Sarashina-Embedding-v1-1B を Hugging Face から取得して使用可能にするラッパ。

参考:
- src/WeatherLLM/llava.py のローカル保存方針
- モデルカード（想定）: https://huggingface.co/sbintuitions/sarashina-embedding-v1-1b

提供関数:
- get_model(device: str = "cpu", local_dir: Optional[str] = None)
    -> SentenceTransformer
- add_prefix_for_task(text: str, side: str, task: str = "sts") -> str
    タスクに応じた推奨の prefix/instruction を付与した文字列を返す（v2 と同等の運用）
...
- Sentence Transformers 5.1.1 / Transformers 4.57.0 を想定
"""
...
LOGGER = setup_logger("sarashina-embedding-v1-1b")
...
# v2 と同等の日本語 instruction を流用（STS は両側を query 形式にする）
...
def _ensure_local_model_dir(
    ...
    if _has_model_files(local_dir):
        LOGGER.info(f"[sarashina-v1] local snapshot found. skip download: {local_dir}")
        return local_dir

    if not HAS_HF_HUB:
        LOGGER.warning(
            "[sarashina-v1] huggingface_hub が見つからないため、ローカル保存はスキップします。"
            "オンラインで SentenceTransformer から直接読み込みます。"
        )
        return model_id

    os.makedirs(local_dir, exist_ok=True)
    LOGGER.info(f"[sarashina-v1] downloading snapshot to: {local_dir}")
    snapshot_download(
        repo_id=model_id,
        local_dir=local_dir,
        token=hf_token,
...
def get_model(
    ...
    """
    Sarashina-Embedding-v1-1B モデルをロードして返す。
    ...
    # （想定）1792 次元 / 最終トークンプーリング
    LOGGER.info(f"[sarashina-v1] model loaded from: {path_or_id} (device={model.device})")
    return model
...
def embed_and_score(
    ...
    - STS タスクでは、両文に同じ instruction + query プレフィックスを付与
...
if __name__ == "__main__":
    ...
    print(f"[self-test] v1 similarity(STS)={sim:.4f}")
